...
def fin_F_array(angle_attack_force_run, start1, end1, step1):  #indexes of excel sheet
  ...
  for i in range(start1, end1, step1):
      angle_attack_wind = (180/np.pi)*np.arctan(3/vz_array[i]) #assume 5m/s crosswind
      # if i>88:
      #     angle_attack_wind = angle_attack_force_run #(180/np.pi)*np.arctan(wind_lookup(z_array[i])/vz_array[i])
      # else:
...
def fin_F_plotter():
    ...
    #plt.axvline(np.argmin(np.abs(mach_array-0.8)<0.01), color = 'red', label = 'Hypersonic M>5', linestyle = '--')
